...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        # Write your code here #
        ########################
        df["weekend_stay"] = df.apply(lambda row: str(pd.date_range(row['date_from'], row['date_to']-pd.Timedelta(days=1), freq='D').dayofweek.isin([4, 5]).any()), axis=1)
        return df
    @staticmethod
    def add_night_price(df):
        ...
        df['night_price'] = round(df['night_price'], 2)
    ...
    def aggregate_group_reservations(self, df):
        ...
        # Write your code here #
        ########################
        sum_cols = group_reservations.groupby("group_id")[self.sum_columns].sum()
        mean_cols = group_reservations.groupby("group_id")[self.mean_columns].mean()
        mode_cols = group_reservations.groupby("group_id")[self.mode_columns].agg(lambda x: x.value_counts().index[0])
        first_cols = group_reservations.groupby("group_id")[self.first_columns].first()
        merged = sum_cols.merge(mean_cols, on="group_id", how='left').merge(mode_cols, on="group_id", how='left').merge(first_cols, on="group_id", how="left")
        final = pd.concat([non_group_reservations, merged])
        return final
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        # Write your code here #
        ########################
        grouped = df[['room_group_id', "night_price"]].groupby("room_group_id").mean().reset_index().rename(columns={"night_price": "mean_price"})
        df['room_segment'] = df.merge(grouped, on="room_group_id", how="left")["mean_price"].apply(
